...
void GameObject::LoadMTL (const char* path)
{
...
void GameObject::LoadObject (const char* path, std::vector<glm::vec3>& verts, std::vector<glm::vec2>& uvs, std::vector<glm::vec3>& norms)
{
	...
	if (inputFile.is_open ())
	{
		...
		while (std::getline (inputFile, currentLine))
		{
			...
			else if (isUsingMtl && !currentLine.substr (0, 6).compare (0, 6, "usemtl"))
			{
				std::string name;
		...
		if (numVertices > 0)
		{
			...
			if (isUsingMtl)
			{
				glGenBuffers (1, &VBO[1]);
				glBindBuffer (GL_ARRAY_BUFFER, VBO[1]);
				glBufferData (GL_ARRAY_BUFFER, sizeof (float) * numVertices * 3, &colourCoords[0], GL_STATIC_DRAW);
